...
public final class JetTvOSSlider: UIControl {
     ...
     */
    public func setValue(_ value: Float, animated: Bool, isActionEnable : Bool) {
        self.value = value
        stopDeceleratingTimer(isActionEnable)
    ...
    private func handleDeceleratingTimer(timer: Timer) {
        ...
        if !isFocused || abs(deceleratingVelocity) < 1 {
            stopDeceleratingTimer(true)
        }
    }
    
    
    private func stopDeceleratingTimer(_ isActionEnable : Bool) {
    ...
    private func leftTapWasTriggered() {
        setValue(value-stepValue, animated: true,isActionEnable : true)
    }
